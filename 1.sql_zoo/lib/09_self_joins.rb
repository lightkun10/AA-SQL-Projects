...
def connecting_routes
  ...
  execute(<<-SQL)
    ...
    FROM
      routes
    WHERE
      routes.stop_id = 149 OR stop_id = 53
    GROUP BY 
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes AS end_routes
        ...
        AND start_routes.num = end_routes.num
    JOIN
      stops AS origin_stops
        ON origin_stops.id = start_routes.stop_id
    JOIN
      stops AS destination_stops
      ON destination_stops.id = end_routes.stop_id
    WHERE
